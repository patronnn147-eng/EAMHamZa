  ...
  return (
    <div className="space-y-6 p-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">Vue lecture seule. Aucune modification n'est possible depuis cet écran.</p>
      </div>
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-1">
                {Object.entries(kpi.failures_by_type).map(([k, v]) => (
                  <div key={k} className="flex justify-between text-xs">
                    <span className="text-gray-600 truncate">{FAILURE_LABELS[k] || k}</span>
                    <span className="font-bold text-gray-800 ml-1">{v}</span>
                  </div>
                ))}
                {Object.keys(kpi.failures_by_type).length === 0 && (
                  <p className="text-xs text-gray-400 col-span-2">Aucune donnée</p>
                )}
              </div>
            </CardContent>
          </Card>
          ...
          {filtered.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b text-left text-xs font-bold text-gray-500 uppercase tracking-wider">
                ...
                <tbody className="divide-y">
                  {filtered.map((wo) => (
                    <tr key={wo.id} className="hover:bg-gray-50 transition-colors">
                      <td className="py-3 pr-4 font-mono font-bold text-gray-700">#{wo.id}</td>
                      <td className="py-3 pr-4 text-gray-700">{wo.machine_nom || `#${wo.machine_id}`}</td>
                      <td className="py-3 pr-4">
                        <div>
                          <p className="text-gray-700">{wo.technician_nom || '—'}</p>
                          {wo.technician_email && <p className="text-xs text-gray-400">{wo.technician_email}</p>}
                        </div>
                      </td>
                      ...
                      <td className="py-3 pr-4">
                        {wo.failure_type ? (
                          <Badge variant="outline" className={wo.failure_type === 'NONE' ? 'text-gray-400' : 'text-amber-700 border-amber-300 bg-amber-50 text-xs'}>
                      ...
                      <td className="py-3 pr-4 max-w-[200px]">
                        {wo.rapport
                          ? <p className="text-xs text-gray-600 line-clamp-2">{wo.rapport}</p>
                          : <span className="text-gray-300">—</span>}
                      </td>
