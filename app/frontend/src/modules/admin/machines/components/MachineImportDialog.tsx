    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-[700px] max-h-[90vh] flex flex-col">
        ...
        <div className="flex flex-col gap-4 flex-1 overflow-hidden">
          <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border border-dashed rounded-md">
            <div className="flex-1">
              <p className="text-sm font-medium text-gray-900">Format Requis (Standardisation)</p>
              <p className="text-sm text-gray-500 max-w-md mt-1">
                Utilisez impérativement le modèle fourni (voir feuille &quot;Guide des Règles&quot;). 
                Les colonnes <strong className="text-gray-700">zone, sous_zone et ordre</strong> sont obligatoires et doivent correspondre aux valeurs exactes. 
                Le <strong className="text-gray-700">nom</strong> est généré automatiquement s'il est laissé vide.
              </p>
            </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <Button 
              ...
              disabled={loading}
              className="w-full h-24 flex flex-col gap-2 items-center justify-center border-2 border-dashed border-gray-300 bg-gray-50 text-gray-600 hover:bg-gray-100 hover:border-gray-400"
              variant="outline"
            >
              <Upload size={24} />
          ...
          {previewItems.length > 0 && (
            <div className="flex-1 overflow-hidden flex flex-col gap-2 min-h-64">
              <div className="flex items-center justify-between text-sm py-2 px-1 border-b">
                <div className="font-semibold text-gray-700">Aperçu ({stats.total} lignes)</div>
              ...
              <div className="overflow-auto flex-1 border rounded min-h-[250px] shadow-inner bg-gray-50 p-2">
                <table className="w-full text-sm text-left">
                  ...
                  <tbody>
                    {previewItems.map((item, i) => (
                      <tr key={i} className={`border-b border-gray-100 ${item.valid ? 'bg-white' : 'bg-red-50'}`}>
                        ...
                        <td className="px-3 py-2 text-gray-600">{item.data?.type || '-'}</td>
                        <td className="px-3 py-2 text-gray-600">{item.data?.zone || '-'}</td>
