...
export const CheckColumn: React.FC<CheckColumnProps> = ({
    ...
    return (
        <div className="flex-1 flex flex-col gap-3 overflow-y-auto no-scrollbar pb-4">
            ...
            {items.length === 0 && (
                <div className="h-24 mt-4 flex flex-col items-center justify-center border-2 border-dashed border-green-800/50 rounded-lg text-green-400 text-xs italic bg-slate-900/40">
                    <span className="not-italic font-medium">Aucun élément</span>
                    <span className="not-italic text-[10px] opacity-70">
                        Terminez un OT pour voir les interventions
                    </span>
                </div>
            )}
        </div>
