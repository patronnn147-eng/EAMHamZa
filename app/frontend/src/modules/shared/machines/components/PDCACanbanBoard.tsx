...
export const PDCACanbanBoard = () => {
    ...
    const getPriorityColor = (p: string) => {
        ...
        return map[p?.toUpperCase()] || 'bg-gray-100 text-blue-100';
    };
    ...
    const renderKanbanCard = (item: KanbanItem) => {
        ...
        switch (item.phase) {
                ...
                return (
                    <motion.div key={item.id} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className={baseCardClasses}>
                        <div className="flex justify-between items-start mb-2">
                            ...
                            <span className="text-[10px] text-blue-400 flex items-center gap-1 font-mono"><Clock className="w-3 h-3" />{new Date(item.date).toLocaleDateString('fr-FR')}</span>
                        </div>
                        <p className="text-xs font-bold text-white line-clamp-2 leading-tight mb-1">{item.title}</p>
                        <p className="text-[10px] text-blue-300 flex items-center gap-1 mb-2"><Activity className="w-3 h-3" />{item.machineName}</p>
            ...
            case 'DO':
                return (
                    <motion.div key={item.id} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className={`${baseCardClasses} ${item.isBlocked ? 'ring-2 ring-red-400 border-red-400' : ''}`}>
                        ...
                        <p className="text-xs font-bold text-white line-clamp-2 leading-tight mb-1">{item.title}</p>
                        <p className="text-[10px] text-blue-300 flex items-center gap-1 mb-2"><Wrench className="w-3 h-3" />{item.machineName}</p>
            ...
            case 'CHECK':
                return (
                    <motion.div key={item.id} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className={baseCardClasses}>
                        ...
                        <div className="space-y-2 mt-auto pt-2 border-t border-gray-50 bg-slate-800/50 -mx-3 px-3 pb-3 -mb-3 rounded-b-lg">
            ...
            case 'ACT':
                return (
                    <motion.div key={item.id} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className={`${baseCardClasses} bg-slate-800/50 border-blue-700/50 opacity-90 hover:opacity-100`}>
                        <div className="flex justify-between items-start mb-2">
                            <Badge className="text-[10px] bg-indigo-100 text-indigo-700 border-indigo-200">En cours d'amélioration</Badge>
                        </div>
                        <p className="text-xs font-bold text-white line-clamp-2 leading-tight mb-1">{item.title}</p>
                        <p className="text-[10px] text-blue-300 flex items-center gap-1 mb-2 italic">Cause: {item.subtitle.replace('Cause:', '')}</p>
                        
                        <div className="space-y-2 mt-auto pt-2 border-t border-blue-800/50">
                            {item.effectiveness && (
                                <div className="grid grid-cols-2 gap-1 mb-2">
                                    <div className="flex items-center gap-1 bg-slate-800 p-1 rounded border border-green-100">
                                        <TrendingUp className="w-3 h-3 text-green-500" />
                                        <span className="text-[9px] text-blue-200">{item.effectiveness.downtimeReduction} d'arrêt</span>
                                    </div>
                                    <div className="flex items-center gap-1 bg-slate-800 p-1 rounded border border-blue-100">
                                        <Zap className="w-3 h-3 text-blue-500" />
                                        <span className="text-[9px] text-blue-200">{item.effectiveness.costSavings} sauvés</span>
                                    </div>
                                </div>
                            )}
                            <div className="flex gap-2">
                                <Button 
                                    ...
                                    className="flex-1 text-[9px] h-7 bg-slate-800 border-indigo-200 text-indigo-700"
                                >
                                    MAJ Gamme
                                </Button>
                                <Button 
                        ...
                        <div key={col.id} className={`flex flex-col rounded-xl border ${col.border} ${col.color} p-4 space-y-4 shadow-sm`}>
                            ...
                            <div className="flex flex-col space-y-1 mb-2 px-1">
                                <div className="flex items-center justify-between">
                                    ...
                                    <Badge variant="outline" className="bg-white/70 backdrop-blur-sm text-blue-100 font-bold px-2.5">
                                        {colItems.length}
                                    </Badge>
                                </div>
