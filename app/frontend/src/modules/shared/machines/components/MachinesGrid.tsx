import React from 'react';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Edit, Trash2 } from 'lucide-react';
import type { Machine } from '@/lib/types';

interface MachinesGridProps {
  machines: Machine[];
  canEdit?: boolean;
  canDelete?: boolean;
  onEdit: (machine: Machine) => void;
  onRequestDelete: (machine: Machine) => void;
}

// Helper function to truncate text with title attribute for tooltip
const TruncateText: React.FC<{ text: string; maxLength: number }> = ({ text, maxLength }) => {
  if (text.length <= maxLength) {
    return <span>{text}</span>;
  }
  
  return (
    <span 
      title={text} 
      className="cursor-help truncate"
    >
      {text.substring(0, maxLength)}...
    </span>
  );
};

export const MachinesGrid: React.FC<MachinesGridProps> = ({
  machines,
  canEdit = true,
  canDelete = false,
  onEdit,
  onRequestDelete,
}) => {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {machines.length === 0 ? (
        <div className="col-span-full text-center py-12">
          <p className="text-blue-300">No machines found</p>
        </div>
      ) : (
        machines.map((machine) => (
          <Card key={machine.id} className="bg-gradient-to-br from-slate-800 to-blue-900 border-blue-700 hover:shadow-lg hover:shadow-blue-500/20 transition-all">
            <CardHeader>
              <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  <CardTitle className="text-lg truncate text-white">
                    <TruncateText text={machine.nom} maxLength={25} />
                  </CardTitle>
                  {machine.ordre && (
                    <p className="text-sm text-blue-300 mt-1">Order: {machine.ordre}</p>
                  )}
                </div>
              </div>
            </CardHeader>
            <CardContent>
              <img
                src={
                  machine.image_url ||
                  'https://mgx-backend-cdn.metadl.com/generate/images/934400/2026-01-27/e1cfe394-7674-4c68-a85b-56fb0119fd9d.png'
                }
                alt={machine.nom}
                className="w-full h-48 object-cover rounded-md mb-4 ring-2 ring-blue-500/30"
              />
              <div className="space-y-2 text-sm mb-4">
                {(machine.zone || machine.sous_zone) && (
                  <div className="flex justify-between">
                    <span className="text-blue-300">Zone:</span>
                    <span className="font-medium">
                      {[machine.zone, machine.sous_zone]
                        .filter(Boolean)
                        .join(' / ')}
                    </span>
                  </div>
                )}
                {machine.date_derniere_maintenance && (
                  <div className="flex justify-between">
                    <span className="text-blue-300">Last Maintenance:</span>
                    <span className="font-medium">
                      {new Date(machine.date_derniere_maintenance).toLocaleDateString()}
                    </span>
                  </div>
                )}
                {machine.date_prochaine_maintenance && (
                  <div className="flex justify-between">
                    <span className="text-blue-300">Next Maintenance:</span>
                    <span className="font-medium text-amber-400">
                      {new Date(machine.date_prochaine_maintenance).toLocaleDateString()}
                    </span>
                  </div>
                )}
              </div>
              {(canEdit || canDelete) && (
                <div className="flex gap-2">
                  {canEdit && (
                    <Button variant="outline" size="sm" className="flex-1 bg-blue-700/40 border-blue-600 text-blue-100 hover:bg-blue-600 hover:text-white" onClick={() => onEdit(machine)}>
                      <Edit className="mr-2 h-4 w-4" />
                      Edit
                    </Button>
                  )}
                  {canDelete && (
                    <Button
                      variant="outline"
                      size="sm"
                      className="flex-1 bg-red-900/40 border-red-700 text-red-200 hover:bg-red-800"
                      onClick={() => onRequestDelete(machine)}
                    >
                      <Trash2 className="mr-2 h-4 w-4" />
                      Delete
                    </Button>
                  )}
                </div>
              )}
            </CardContent>
          </Card>
        ))
      )}
    </div>
  );
};
