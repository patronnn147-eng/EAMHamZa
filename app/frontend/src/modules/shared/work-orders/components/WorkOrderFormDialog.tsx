    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
                ...
                <div className="grid gap-2">
                  ...
                  <div className="border rounded-md p-3 max-h-48 overflow-y-auto space-y-2 bg-slate-800/50">
                    {planningTechnicians.length === 0 ? (
                      <p className="text-sm text-blue-300">No technicians in this planning</p>
                    ) : (
                      planningTechnicians.map((t) => (
                        <div key={t.id} className="flex items-center space-x-2 p-2 hover:bg-slate-800 rounded">
                          <Checkbox
                            id={`tech-${t.id}`}
        ...
        <DialogFooter>
          ...
          <Button onClick={onSubmit} className="bg-blue-600 hover:bg-blue-700 text-white">{editingWorkOrder ? 'Update' : 'Create'}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
