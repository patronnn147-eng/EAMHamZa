      ...
      {kpi && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <Card className="border-none shadow bg-gray-50">
            <CardContent className="pt-4 pb-3">
              <p className="text-xs font-bold text-gray-500 uppercase tracking-wider">Pannes / Type</p>
              <div className="mt-1 space-y-0.5">
                {Object.entries(kpi.failures_by_type).map(([k, v]) => (
                  <div key={k} className="flex justify-between text-xs">
                    <span className="text-gray-600">{FAILURE_LABELS[k] || k}</span>
                ...
                <tbody className="divide-y">
                  {filtered.map((wo) => (
                    <tr key={wo.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="py-3 pr-4 text-gray-700">{wo.technician_nom || '—'}</td>
                      ...
                      <td className="py-3 pr-4">
                        {wo.failure_type ? (
                          <Badge variant="outline" className={wo.failure_type === 'NONE' ? 'text-gray-500' : 'text-amber-700 border-amber-300 bg-amber-50'}>
                      ...
                      <td className="py-3 pr-4 max-w-[200px]">
                        {wo.rapport ? (
                          <p className="text-xs text-gray-600 line-clamp-2 leading-relaxed">{wo.rapport}</p>
                        ) : <span className="text-gray-300">—</span>}
                      </td>
