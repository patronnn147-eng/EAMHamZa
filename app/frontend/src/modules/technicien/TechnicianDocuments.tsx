...
export default function TechnicianDocuments() {
  ...
  const getTypeColor = (type: string) => {
    ...
    switch (type.toLowerCase()) {
      case 'manuel':
        return base + 'text-blue-700 bg-blue-50/50 border-blue-200/50';
      case 'schema':
        return base + 'text-purple-700 bg-purple-50/50 border-purple-200/50';
      case 'procedure':
        return base + 'text-green-700 bg-green-50/50 border-green-200/50';
      case 'certification':
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-4xl font-bold text-white">Documents Techniques</h2>
        <p className="mt-1 text-sm text-blue-300">Accédez aux manuels, schémas et procédures</p>
      </div>
        ...
        {filteredDocuments.length === 0 ? (
          <Card className="col-span-full bg-slate-800/80 backdrop-blur-md border border-blue-800/30 shadow-xl">
            <CardContent className="text-center py-12">
              <p className="text-blue-300">Aucun document trouvé</p>
            </CardContent>
          </Card>
        ) : (
          filteredDocuments.map((doc) => (
            <Card key={doc.id} className="bg-slate-800/80 backdrop-blur-md border border-blue-800/30 hover:shadow-xl hover:border-blue-700/50 transition-all">
              <CardHeader>
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-blue-300 mt-1">{doc.identifiant_archive}</p>
                  </div>
