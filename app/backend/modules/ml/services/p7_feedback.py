...
def compute_feedback_metrics(
    ...
    if not predicted_names and not actual_names:
        return {"precision": None, "recall": None, "f1": None,
                "predicted_count": 0, "actual_count": 0, "tp": 0}
...
async def record_p7_feedback(
    ...
    if not itv or not parts_text:
        logger.debug(f"[P7-feedback] Intervention {intervention_id}: no parts text data")
        return None
    ...
    if not pred_log:
        logger.debug(f"[P7-feedback] No p7_parts_demand log for machine {itv.machine_id}")
        return None
