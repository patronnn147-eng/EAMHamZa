...
async def get_completed_work_orders(
    *, page: Annotated[int, Query(ge=1)] = 1,
    ...
    query = select(OrdresTravail).where(
        OrdresTravail.statut.in_(
            [OrdreStatut.COMPLETED, OrdreStatut.VALIDATED, OrdreStatut.CLOSED]
        )
    )
    query = query.options(
        selectinload(OrdresTravail.machine),
        selectinload(OrdresTravail.utilisateur),
    )

    if technician_id:
        query = query.where(OrdresTravail.utilisateur_id == technician_id)
    if machine_id:
        query = query.where(OrdresTravail.machine_id == machine_id)
    if failure_type:
        query = query.where(OrdresTravail.failure_type == failure_type)
    if date_from:
        query = query.where(OrdresTravail.date_fin >= date_from)
    if date_to:
        query = query.where(OrdresTravail.date_fin <= date_to)

    query = query.order_by(OrdresTravail.date_fin.desc()).offset(skip).limit(size)
...
async def get_kpi_report(
    *, date_from: Annotated[Optional[datetime], Query()] = None,
    ...
    base_query = select(OrdresTravail).where(
        OrdresTravail.statut.in_(
            [OrdreStatut.COMPLETED, OrdreStatut.VALIDATED, OrdreStatut.CLOSED]
        )
    )
    ...
    if date_from:
        base_query = base_query.where(OrdresTravail.date_fin >= date_from)
    if date_to:
        base_query = base_query.where(OrdresTravail.date_fin <= date_to)
...
async def get_cheftech_analytics_dashboard(
    ...
    i_result = await db.execute(
        ...
        .options(selectinload(OrdresIntervention.technician))
    )
