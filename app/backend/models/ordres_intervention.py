...
class Ordres_intervention(Base):
    ...
    requested_by = Column(
        Integer, nullable=True
    )  # User who requested the intervention (ChefOp)
    ...
    actual_failure_type = Column(
        String(20), nullable=True
    )  # TWF, HDF, PWF, OSF, RNF, or None
    ml_prediction_matched = Column(
        Boolean, nullable=True
    )  # Did the ML prediction match reality?
    retrained = Column(
    ...
    )  # Running, Idle, Startup, Shutdown
    temperature = Column(String(50), nullable=True)
    impact = Column(
        String(100), nullable=True
    )  # Production stopped, Reduced performance, No impact yet
    ...
    root_cause_category = Column(
        String(50), nullable=True
    )  # Mechanical, Electrical, Software, Human error, Unknown
    ...
    machine_status_after = Column(
        String(50), nullable=True
    )  # Operational, Limited, Stopped
    ...
    check_verification_method = Column(
        String(100), nullable=True
    )  # Test run, Monitoring, Visual inspection
