...
class UserService:
    ...
    async def update_user_profile(
        db: AsyncSession, id: str, name: Optional[str] = None
        ...
        user = result.scalar_one_or_none()
        logger.debug(
            f"[DB_OP] User lookup completed in {time.time() - start_time:.4f}s - found: {user is not None}"
        )
        ...
        if user and name is not None:
            ...
            logger.debug(
                f"[DB_OP] User profile update completed in {time.time() - start_time_update:.4f}s"
            )
