...
async def get_admin_analytics_dashboard(
    ...
    for wo in completed_wos:
        ...
        if wo.date_debut and wo.date_fin:
            tech_stats[tid]["total_duration"] += int((wo.date_fin - wo.date_debut).total_seconds() / 60)
    ...
    trend_chart = [
        ...
        for date_str, counts in sorted(req_trends.items())
    ]
...
async def export_admin_analytics(
    ...
    for wo in all_wos:
        ...
        date_echeance = wo.date_echeance.strftime("%Y-%m-%d %H:%M:%S") if wo.date_echeance else ""
    ...
    return Response(
        content=csv_content,
        media_type="text/csv",
        headers={"Content-Disposition": f"attachment; filename=export_work_orders_{datetime.now().strftime('%Y%m%d%H%M')}.csv"}
    )
