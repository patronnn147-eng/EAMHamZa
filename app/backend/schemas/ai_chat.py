...
class ChatResponse(BaseModel):
    ...
    tool_calls: Optional[List[ToolCall]] = Field(
        default=None, description="Tool calls requested by AI"
    )
    sources: Optional[List[Dict[str, Any]]] = Field(
        default=None, description="Data sources used for response"
    )
    session_id: Optional[str] = Field(
        default=None, description="ChatSession UUID this exchange landed in"
    )
    session_title: Optional[str] = Field(
        default=None, description="Current title of the session (may auto-update)"
    )
