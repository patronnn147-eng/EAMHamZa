...
async def _clean_seed_data(db) -> None:
    ...
    # Collect seed OT ids first (used as FK in ITV + planning_OrdresTravail)
...
async def seed():
    ...
    async with db_manager.async_session_maker() as db:
        ...
        logger.info(
            f"  {NUM_CYCLES * 2} PlanningTaches (DIAGNOSTIC + CORRECTION × {NUM_CYCLES})"
        )
        ...
        logger.info(
            "  1. Open localhost:3000/machines/20 → sensors + failure prob > 0%"
        )
