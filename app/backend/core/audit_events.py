...
async def audit_after_flush(mapper: Mapper, connection, flush_event: Any):
    ...
    from services.audit import AuditService, AuditActionType
...
def setup_audit_listeners():
    ...
    @event.listens_for(inspect(inspect) if hasattr(inspect, 'mapped_table') else object, "after_flush")
            ...
            try:
                ...
                if entity_type and session.is_modified(obj):
                    old_values = {
                        ...
                        if col.key in inspect(obj).attrs and inspect(obj).attrs[col.key].history.deleted
                        else None
                        for col in mapper.columns
                        if not col.key.startswith('_')
                    }
