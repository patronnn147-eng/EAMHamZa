...
class NotificationBroadcaster:
    ...
    def subscribe(self, user_id: int) -> asyncio.Queue:
        ...
        logger.debug(
            f"User {user_id} subscribed to notifications. Total queues: {len(self.user_queues[user_id])}"
        )
        return queue
    ...
    async def broadcast(self, user_id: int, notification_data: dict):
        if user_id in self.user_queues:
            ...
            tasks = [
                queue.put(notification_data) for queue in self.user_queues[user_id]
            ]
            if tasks:
                await asyncio.gather(*tasks)
            logger.debug(
                f"Broadcasted notification to user {user_id} (found {len(tasks)} active queues)"
            )
