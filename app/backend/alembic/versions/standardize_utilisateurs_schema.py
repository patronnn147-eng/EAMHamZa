...
def upgrade() -> None:
    ...
    for col_name, col_type in [
        ...
        if not exists:
            op.add_column(
                "utilisateurs", sa.Column(col_name, sa.String(255), nullable=True)
            )
    ...
    if not index_exists:
        op.create_index(
            "ix_utilisateurs_email", "utilisateurs", ["email"], unique=False
        )
...
def downgrade() -> None:
    ...
    op.add_column(
        "utilisateurs", sa.Column("id", sa.String(), nullable=False, server_default="")
    )
    op.add_column(
        "utilisateurs", sa.Column("nom", sa.String(), nullable=False, server_default="")
    )
    op.add_column(
        "utilisateurs",
        sa.Column("mot_de_passe", sa.String(), nullable=False, server_default=""),
    )
    op.add_column(
        "utilisateurs",
