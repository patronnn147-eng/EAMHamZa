...
def upgrade():
    if not _column_exists("planning_taches", "statut"):
        op.add_column("planning_taches", sa.Column("statut", sa.String(20), nullable=False, server_default="DRAFT"))

    if not _column_exists("ordres_intervention", "planning_tache_id"):
        op.add_column("ordres_intervention", sa.Column("planning_tache_id", sa.Integer(), nullable=True))
