...
def upgrade() -> None:
    ...
    if not op.get_context().bind.dialect.has_table(op.get_context().bind, 'ai_memories'):
        op.create_table(
            'ai_memories',
            sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True, default=sa.text('gen_random_uuid()')),
            sa.Column('utilisateur_id', sa.Integer(), sa.ForeignKey('utilisateurs.id'), nullable=False, index=True),
        ...
        op.create_index('ix_ai_memories_user_type', 'ai_memories', ['utilisateur_id', 'memory_type'])
