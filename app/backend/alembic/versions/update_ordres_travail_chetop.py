...
def upgrade():
    ...
    op.add_column('ordres_travail', sa.Column('updated_at', sa.DateTime(timezone=True), nullable=True))
    
    # Drop old ordre_id column if it exists
    op.drop_column('ordres_travail', 'ordre_id', nullable=True)
    
    # Update default values
    op.alter_column('ordres_travail', 'priorite', 
                    existing_type=sa.String(),
                    server_default='MOYENNE',
                    nullable=False)
    
    op.alter_column('ordres_travail', 'statut',
                    existing_type=sa.String(),
                    server_default='EN_ATTENTE',
                    nullable=False)
    
    op.alter_column('ordres_travail', 'date_echeance',
                    existing_type=sa.DateTime(timezone=True),
                    nullable=True)
    
    op.alter_column('ordres_travail', 'utilisateur_id',
                    existing_type=sa.Integer(),
                    nullable=True)
    
    # Make created_at not nullable with default
    op.alter_column('ordres_travail', 'created_at',
                    existing_type=sa.DateTime(timezone=True),
