...
class ModelRegistry:
    ...
    def get(self, model_name: str, version: str = None) -> Optional[Dict]:
        ...
        if matching:
            ...
            matching.sort(key=lambda x: x.get("registered_at", ""), reverse=True)
            return matching[0]
    ...
    def list_versions(self, model_name: str) -> list:
        ...
        """
        return [v for k, v in self.models.items() if k.startswith(f"{model_name}:")]
...
def register_model(
    model_name: str, version: str, metrics: Dict = None, model_path: str = None
) -> Dict:
