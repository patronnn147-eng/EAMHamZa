...
## Corpus Check
- 6176 files · ~99,999 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 6176 nodes · 14217 edges · 252 communities detected
- Extraction: 59% EXTRACTED · 41% INFERRED · 0% AMBIGUOUS · INFERRED: 5814 edges (avg confidence: 0.5)
...
Cohesion: 0.01
Nodes (660): Config, get_all_pending_requests(), _handle_itv_approved(), _handle_itv_rejected(), ItvRequestResponse, ItvRequestValidation, Admin: Approve or Reject an intervention request, Mark intervention as DECLINED and release any prior stock reservation. (+652 more)

### Community 2 - "Community 2"
Cohesion: 0.01
Nodes (285): AnomalyEnsemble, CUSUMDetector, fit_anomaly_ensemble(), Model E: Isolation Forest + CUSUM Control Chart Anomaly Ensemble Dual-gate: BOTH, Fit the ensemble on healthy (in-control) data.          Args:             X: (n_, Create fresh (zero-state) CUSUM detectors from stored baselines., Replay history observations through temp_cusums (mutates in place)., Score current observation x against per-feature CUSUM detectors. (+277 more)

### Community 3 - "Community 3"
Cohesion: 0.01
Nodes (268): _apply_filters(), _apply_sort(), Archives, ArchivesBatchCreateRequest, ArchivesBatchDeleteRequest, ArchivesBatchUpdateItem, ArchivesBatchUpdateRequest, ArchivesData (+260 more)

### Community 4 - "Community 4"
Cohesion: 0.02
Nodes (236): _enum_val(), _get_machine_ids(), get_planning_with_users(), get_user_by_id(), _load_users_from_db(), _load_users_preloaded(), Validate shift_type / chef_operation_id consistency for SHIFT plannings., Raise 404 if user not found, 400 if role mismatch. (+228 more)

### Community 5 - "Community 5"
Cohesion: 0.02
Nodes (206): get_availability(), get_availability_batch(), Live availability endpoints — read-only observability for stock + reservations., Return raw stock, reserved total, and computed available qty for one piece., Batch availability lookup — single query, returns one row per input id., ConsumedPiece, ConsumedPiece model — actual piece consumption recorded at WO completion.  One, _build_response() (+198 more)

### Community 6 - "Community 6"
Cohesion: 0.01
Nodes (164): _autofit_columns(), _build_admin_export_flat_row(), export_work_order_report(), _fetch_consumed_summary(), _itv_str(), Export a complete Work Order report to Excel (horizontal format, analysis-ready), Export a complete Work Order report to Excel (horizontal format, analysis-ready), _fallback_plan() (+156 more)

### Community 7 - "Community 7"
Cohesion: 0.01
Nodes (145): embed_batch(), embed_cache_stats(), embed_text(), get_embedder(), Embedding model singleton.  SentenceTransformer.encode() is synchronous — call v, Sync batch embed — run in thread pool, never call directly in async context., Async-safe batch embed. Runs sync encode in thread pool executor., Async-safe single text embed with in-memory TTL cache.      Cache key = raw text (+137 more)

### Community 8 - "Community 8"
Cohesion: 0.02
Nodes (144): Config, create_access_token(), decode_access_token(), get_admin_user(), get_bearer_token(), get_current_user(), get_me(), login() (+136 more)
...
Cohesion: 0.03
Nodes (113): ChatMessage, ChatRequest, ChatResponse, ChatSessionSummary, CreateSessionRequest, Chat schemas for AI chat endpoint, Request to AI chat endpoint, Tool call requested by AI (+105 more)
...
Cohesion: 0.04
Nodes (77): _apply_filters(), _apply_sort(), check_and_send_due_reports(), _collect_recipients(), Config, create_rapports(), create_rapportss_batch(), delete_rapports() (+69 more)
...
Cohesion: 0.04
Nodes (70): _drift_rows(), get_ml_model_metrics(), get_retraining_stats(), ml_service_status(), model_health(), trigger_retraining(), _count_recent_interventions(), _entries_to_log_dicts() (+62 more)
...
Cohesion: 0.06
Nodes (52): hybrid_enabled(), hybrid_stats(), keyword_search(), Phase 13.1 hybrid retrieval: keyword (Postgres FTS via two GENERATED tsvector co, Postgres FTS keyword search across content_tsv_fr + content_tsv_en.      Returns, Fuse two ranked lists by Reciprocal Rank Fusion (Cormack 2009).      Each input, Toggle: env HYBRID_ENABLED, default true., Update module-level counters after a hybrid query completes.      Args: (+44 more)
...
Cohesion: 0.08
Nodes (20): verify_cheftech_only(), _apply_intervention_dates(), create_intervention_request(), get_interventions(), get_my_intervention_requests(), _notify_status_change(), _propagate_wo_status(), ChefOp requests an intervention (PDS) (+12 more)
...
Cohesion: 0.16
Nodes (21): bulk_upload_documents(), BulkUploadResponse, delete_document(), DocumentResponse, download_document(), _ingest_one(), list_documents(), RAG document management API.  File blobs live in MinIO bucket `rag-docs`; meta (+13 more)
...
Cohesion: 0.18
Nodes (6): delete_notification(), NotificationBroadcaster, Delete a notification (only if it belongs to current user), Delete a notification (only if it belongs to current user), Manages SSE connections for notifications, Manages SSE connections for notifications
...
Cohesion: 0.28
Nodes (7): Run migrations in 'online' mode using a synchronous engine., Run migrations in 'online' mode using a synchronous engine., Run migrations in 'online' mode using an asynchronous engine., Run migrations in 'online' mode using an asynchronous engine., run_migrations(), run_migrations_online_async(), run_migrations_online_sync()
...
Cohesion: 0.29
Nodes (5): UtilisateurZones Model - many-to-many zone assignment for CHEFTECH/TECHNICIEN ov, One row per (user, zone) assignment. A user can be assigned to multiple zones., _table_exists(), upgrade(), UtilisateurZone
...
Cohesion: 0.4
Nodes (1): Pure test: pydantic validation only. Importing technicien_work_orders.py pulls i
...
Cohesion: 0.67
Nodes (3): approve_user(), Approve a pending user registration (Admin only), Approve a pending user registration (Admin only)

### Community 164 - "Community 164"
Cohesion: 0.67
Nodes (3): Update alert configuration (admin only), Update alert configuration (admin only), update_alert_config()

### Community 165 - "Community 165"
Cohesion: 0.67
Nodes (2): Mark a notification as read (only if owned by user), Mark a notification as read (only if owned by user)
...
## Knowledge Gaps
- **947 isolated node(s):** `Verification script for P3 RUL Estimation. Tests the MachineLearningService int`, `AWS Lambda handler for unified frontend and backend with Nginx reverse proxy Th`, `Format traceback with newlines replaced by '\\n' string literal`, `Initialize dynamic routes by scanning frontend dist directory`, `Initialize all services once for the Lambda function (equivalent to FastAPI life` (+942 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 172`** (2 nodes): `check_role_schema.py`, `check_schema()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 173`** (2 nodes): `check_zone_travail.py`, `check_zone_travail()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 174`** (2 nodes): `create_minio_bucket.py`, `create_bucket()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 175`** (2 nodes): `create_minio_bucket_manual.py`, `create_bucket_via_curl()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 176`** (2 nodes): `create_minio_bucket_mc.py`, `create_bucket_with_mc()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 177`** (2 nodes): `create_minio_bucket_mc_docker.py`, `create_bucket_via_docker()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 178`** (2 nodes): `create_minio_bucket_mc_docker_fixed.py`, `create_bucket_via_docker()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 179`** (2 nodes): `create_minio_bucket_mc_fixed2.py`, `create_bucket_via_docker()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 180`** (2 nodes): `create_minio_bucket_mc_fixed3.py`, `create_bucket_via_docker()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 181`** (2 nodes): `create_minio_bucket_simple.py`, `create_bucket()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 182`** (2 nodes): `create_tech_tables.py`, `create_tables()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 183`** (2 nodes): `fix_alembic.py`, `main()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 184`** (2 nodes): `inspect_db.py`, `main()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 185`** (2 nodes): `inspect_db_async.py`, `main()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 186`** (2 nodes): `planning_emails.py`, `send_planning_assignment_emails()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 187`** (2 nodes): `aspect-ratio.tsx`, `tailwind.config.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 188`** (2 nodes): `ChefTechMachinesHeader.tsx`, `ChefTechMachinesHeader()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 189`** (2 nodes): `index.ts`, `WelcomePage()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 190`** (2 nodes): `predict_cli.py`, `predict_failure()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 191`** (2 nodes): `ml_train_p2.py`, `P2 — Failure Type Classification (Optimized) Training script: MultiOutputClassif`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 192`** (2 nodes): `ml_train_p4.py`, `P4 — Anomaly Detection (Optimized) Training script: Isolation Forest with GridS`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 193`** (2 nodes): `test_failure_probability.py`, `test_failure_probability_endpoint()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 194`** (2 nodes): `test_p2_failure_type.py`, `test_failure_type_endpoint()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 195`** (2 nodes): `TC001_get_health_basic_check.py`, `test_get_health_basic_check()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 196`** (2 nodes): `TC002_get_api_v1_health_check_database_connected.py`, `test_get_api_v1_health_check_database_connected()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 197`** (2 nodes): `TC006_get_api_v1_ml_machines_machineid_prediction_success.py`, `test_get_api_v1_ml_machines_machineid_prediction_success()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 198`** (2 nodes): `TC007_post_api_v1_ml_retrain_trigger.py`, `test_post_api_v1_ml_retrain_trigger()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 199`** (2 nodes): `Frontend Dependency: axios`, `Frontend Requirements (axios)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 200`** (2 nodes): `check_model_sync.py`, `CI gate: fail if the two model dirs diverge. Exit 1 on divergence, else 0.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 201`** (2 nodes): `planning_tache_emails.py`, `send_task_assignment_email()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 202`** (2 nodes): `patch_predictions.py`, `One-shot patch: add rpm_torque auto-derivation to predict_priority.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 203`** (2 nodes): `patch_predictions2.py`, `Patch predictions.py: auto-derive missing features for P3/P4/P6.  P3 (RUL)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 204`** (2 nodes): `patch_router.py`, `Patch router.py to handle numpy scalar serialization with pydantic + numpy 2.x.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 205`** (2 nodes): `failure_probability.test.py`, `test_failure_probability_endpoint()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 206`** (2 nodes): `p2_failure_type.test.py`, `test_failure_type_endpoint()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 207`** (1 nodes): `analyze_zones.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 208`** (1 nodes): `check_migrations.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 209`** (1 nodes): `update_nb_p4.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 210`** (1 nodes): `write_migration.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 211`** (1 nodes): `Generate backend URL from host and port.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 212`** (1 nodes): `Handle missing enum values by trying to match against string values.         Th`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 213`** (1 nodes): `Handle missing enum values by trying to match against integer values.         T`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 214`** (1 nodes): `piece_machine.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 215`** (1 nodes): `Generate SHAP explanations for a single prediction.                  Args:`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 216`** (1 nodes): `Map dataset column names to user-friendly French labels.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 217`** (1 nodes): `Validate password requirements`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 218`** (1 nodes): `Validate name is not empty`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 219`** (1 nodes): `eslint.config.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 220`** (1 nodes): `postcss.config.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 221`** (1 nodes): `vite.config.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 222`** (1 nodes): `build.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 223`** (1 nodes): `vite-env.d.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 224`** (1 nodes): `collapsible.tsx`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 225`** (1 nodes): `ml_eda.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 226`** (1 nodes): `Favicon SVG — Sagemcom/EAM App Icon (White Stylized S-like Curve on Black Background, 16x16)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 227`** (1 nodes): `ITV Approval Bug (Not Converting to Work Order)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 228`** (1 nodes): `Chef d'Op ITV Request False Error Message`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 229`** (1 nodes): `Deprecated Field 'Niveau de charge (%)' Removal`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 230`** (1 nodes): `Design System Plan (Asana/Monday.com Style)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 231`** (1 nodes): `Database Schema Plan (All Tables)`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 232`** (1 nodes): `Frontend Robots.txt`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 233`** (1 nodes): `mark_safe.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 234`** (1 nodes): `Generate backend URL from host and port.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 235`** (1 nodes): `Validate password requirements`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 236`** (1 nodes): `Build human-readable drift summary string.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 237`** (1 nodes): `Factory method: create monitor from training data.          Args:             X_`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 238`** (1 nodes): `Extract 5 features: [air, process, rpm, torque, wear]                  Args:`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 239`** (1 nodes): `Extract 6 features with temp_delta.                  Args:             teleme`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 240`** (1 nodes): `Validate and sanitize telemetry input.                  Args:             tel`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 241`** (1 nodes): `Check if a value is in valid range.                  Args:             value:`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 242`** (1 nodes): `Compute derived features from raw telemetry.                  Args:`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 243`** (1 nodes): `Get feature names for each model.          Returns:             Dict mapping`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 244`** (1 nodes): `Build a complete FeatureSnapshot dict from telemetry + optional context.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 245`** (1 nodes): `Build a time-ordered list of FeatureSnapshot dicts from prediction log entries.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 246`** (1 nodes): `Infer mean cadence (days) between log entries; default 1/24 (hourly).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 247`** (1 nodes): `Generate SHAP explanations for a single prediction.          Args:`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 248`** (1 nodes): `Convenience wrapper: SHAP for P1 model using canonical 5-feature names.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 249`** (1 nodes): `Map dataset column names to user-friendly French labels.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 250`** (1 nodes): `5 raw sensor features. Used by P3 and P4.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 251`** (1 nodes): `7 features: 5 raw + temp_delta + rpm_torque. Used by P1, P2, P5, P6.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `Utilisateurs` connect `Community 1` to `Community 2`, `Community 3`, `Community 4`, `Community 5`, `Community 6`, `Community 8`, `Community 41`, `Community 11`, `Community 13`?**
  _High betweenness centrality (0.072) - this node is a cross-community bridge._
- **Why does `Machines` connect `Community 1` to `Community 2`, `Community 35`, `Community 3`, `Community 4`, `Community 6`, `Community 5`, `Community 13`?**
  _High betweenness centrality (0.048) - this node is a cross-community bridge._
- **Why does `services/inventory.py (350 lines, to be split)` connect `Community 9` to `Community 5`?**
...
- **Are the 333 inferred relationships involving `Machines` (e.g. with `Base` and `ItvRequestValidation`) actually correct?**
  _`Machines` has 333 INFERRED edges - model-reasoned connections that need verification._
- **Are the 238 inferred relationships involving `UserRole` (e.g. with `Base` and `ItvRequestValidation`) actually correct?**
  _`UserRole` has 238 INFERRED edges - model-reasoned connections that need verification._
- **Are the 199 inferred relationships involving `OrdresIntervention` (e.g. with `Base` and `ItvRequestValidation`) actually correct?**
  _`OrdresIntervention` has 199 INFERRED edges - model-reasoned connections that need verification._